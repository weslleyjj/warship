/*
	...
	Aluno do TADS UFRN EAJ - macaíba
...
void teladejogo(){
	...
	for(int i=0;i<tam;i++){						//laço para impressão dos tabuleiros lado a lado
        ...
        for(int j=0;j<tam;j++){
            ...
            printf(" %c ", tabuleirotela[i][j]);     //pode alterar para tabuleiro cpu para efetuar testes rápidos
			if(i==1 && j==9){
...
void gameplay(){
	...
	do{	
		...
		do{
			...
			coluna=rand()%10;
			colunacp=coluna;
			if(tabuleiro[linha][coluna]=='#'){		//cpu ganha chances extras de acordo com o nível de dificuldade
				break;
...
void atribuir(int linha, int coluna, int pos, int navio, char tab[tam][tam]){		//função para atribuir o navio na posição oferecida
	if(pos==0){	
...
int liberacao(int linha, int coluna, int pos, int navio, char tab[tam][tam]){		//função para verificar a posição dada
	int liberar=0;
	if(pos==0){
		for(int i=0;i<navio;i++){
...
void game(){
    ...
    do{	
		printf("\n\n\t\t\tDigite o nível de dificuldade(0 - Fácil à 5 - Dificil): ");
    	scanf("%d", &IA);
    	if(IA==30){
    		break;
		}
...
void instrucao(){
	...
	printf("\n\n\t  Primeiramente você deverá colocar seus navios no tabuleiro a mostra apresentando\n\t");
	printf("em qual coordenada de LINHA e COLUNA ele será preenchido junto com a direção\n\t");
	printf("que pode ser HORIZONTAL ou VERTICAL e que serão colocados á direita ou abaixo da posição inicial.\n");
    ...
    printf("BÔNUS: Digite 5 no menu para ver as dicas secretas!\n\n\t");
}
void creditos(){
    system("cls");
    printf("\t\t     ____        __        ____             _   __                  __\n");
    ...
    printf("\t\tBaseado em: Batalha Naval de tabuleiro\n\t\tInício do desenvolvimento: 15/05/2019  Término do desenvolviemnto: 10/06/2019 \n\n\n");
}
void dicas(){
	int senha;
	system("cls");
	printf("\n\n\t\tPara visualizar as preciosas dicas digite quantas letras há no nome do orientador do jogo!\n\tResposta: ");
	scanf("%d", &senha);
	if(senha==trava){
		...
		printf("\n\t1 - Os navios não ficam lado a lado, logo evite atirar em posições ao lado de outras ja atingidas");
		printf("\n\t2 - Começar a partida atirando nas diagonais facilita que encontre navios inimigos");
		printf("\n\t3 - Quanto maior a distância entre seus navios mais dificil fica para a CPU encontrá-los");
		printf("\n\t4 - Alterne bastante entre posições verticais e horizontais, isso confunde a CPU e lhe dará tempo");
		printf("\n\t5 - Apesar de tudo 60%% do jogo é sorte");
